...
def main():
    ...
    def init_user(message):
        ...
        bot.register_next_step_handler(message, add_more_vk_group)
    ...
    @bot.message_handler(commands=['add_more_vk_group'])
    def add_more_vk_group(message):
        if message.text.lower() == 'да':
            ...
            check_last_groups_post(message)
            parse_vk_group_posts(message)
        else:
            message = bot.send_message(message.chat.id, f"Только да или нет")
            bot.register_next_step_handler(message, add_more_vk_group)


    @bot.message_handler(commands=['check_last_groups_post'])
    def check_last_groups_post(message):
        with sqlite3.connect('db/database.db') as db:
            cursor = db.cursor()
            vk_all_groups_user = f"""SELECT vk_group_name FROM vk_user_group WHERE FK_telegram_userid={message.from_user.id}"""
            cursor.execute(vk_all_groups_user)
            vk_all_groups_user = cursor.fetchall()

            for vk_user_group in vk_all_groups_user:
                ...
                vk_last_post_dict[vk_user_group] = vk_last_post['id']

        parse_vk_group_posts(message)

    @bot.message_handler(commands=['test'])
    def parse_vk_group_posts(message):
        while True:
            with sqlite3.connect('db/database.db') as db:
                cursor = db.cursor()
                vk_all_groups_user = f"""SELECT vk_group_name FROM vk_user_group WHERE FK_telegram_userid={message.from_user.id}"""
                    ...
                    if 'attachments' in vk_last_post:
                        ...
                        for attachment in attachments_last_post:
                            if attachment['type'] == 'photo':
                                ...
                                all_photos.append(InputMediaPhoto(photos[-1]['url'], vk_last_post["text"])) \
                                    if len(all_photos) == 0 \
                                ...
                                if 'photo' in attachment['link']:
                                    ...
                                    photos = sorted(photos, key=lambda d: d['width'])
                                    all_links.append(
                                        InputMediaPhoto(photos[-1]['url'], f'{vk_last_post["text"]}\n{vk_link_url}')) \
                                        if len(all_links) == 0 \
                            ...
                            #     contains_video = True
                        if vk_last_post_dict[vk_user_group] < vk_last_post['id']:
                            if contains_photo:
                        ...
                        if vk_last_post_dict[vk_user_group] < vk_last_post['id']:
                            bot.send_message(message.chat.id, f'{vk_last_post["text"]}')
                            vk_last_post_dict[vk_user_group] = vk_last_post['id']
