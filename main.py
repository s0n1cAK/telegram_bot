...
def main():
    ...
    def init_user(message):
        ...
        try:
            ...
            pass
        bot.send_message(message.chat.id, f'Хочешь добавить группу вк? (Да/Нет)')
    ...
    def save_vk_group(message):
        try:
            if re.match('https://vk.com/([а-яА-Яa-zA-Z0-9]{1,9999}[^:])', message.text) != None \
                ...
                bot.send_message(message.chat.id, f'Будем ещё группы добавлять? (Да/Нет)')
                bot.register_next_step_handler(message, vk_add_more_group)
                ...
                bot.send_message(message.chat.id, f'Введи группы вк, которую хочешь добавить.')
    ...
    def vk_add_more_group(message):
        if message.content_type == 'text' and message.text.lower() == 'да':
            bot.send_message(message.chat.id, 'Вводи ссылку')
            bot.register_next_step_handler(message, save_vk_group)
            ...
            bot.send_message(message.chat.id, f"Только да или нет")
            bot.register_next_step_handler(message, vk_add_more_group)
